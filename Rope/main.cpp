...
void StudentTests(){
	 ...
	 */
	//TESTING SPECIAL CASES FOR SPLIT
	//SPLIT right->right->left is_leaf()==true
	Rope r1(IndexTestRope()); //Wikipedia split @ 11
	std::string s1 = "Hello my name is Simon";
	Rope r2;
	std::string s2 = s1.substr(0,9);
	std::string s3 = s1.substr(9,9);
	r1.split(9,r2);
	r1.print_strings(std::cout);
	r2.print_strings(std::cout);
	std::cout<<"Passed index 9 test!"<<std::endl<<std::endl;

	//SPLIT left->left->right is_leaf()==true
	Rope r3(IndexTestRope()); //Wikipedia split @ 11
	std::string s4 = "Hello my name is Simon";
	Rope r4;
	std::string s5 = s4.substr(0,6);
	std::string s6 = s4.substr(6,6);
	r3.split(6,r4);
	r3.print_strings(std::cout);
	r4.print_strings(std::cout);
	std::cout<<"Passed index 6 test!"<<std::endl<<std::endl;

	//SPLIT left->left->left is_leaf()==true
	Rope r5(IndexTestRope()); //Wikipedia split @ 11
	std::string s7 = "Hello my name is Simon";
	Rope r6;
	std::string s8 = s7.substr(0,0);
	std::string s9 = s7.substr(0,0);
	r5.split(0,r6);
	r5.print_strings(std::cout);
	r6.print_strings(std::cout);
	std::cout<<"Passed index 0 test!"<<std::endl<<std::endl;

	//SPLIT mid-node (NOT WORKING)
	// Rope r7(IndexTestRope()); //Wikipedia split @ 11
	// std::string s10 = "Hello my name is Simon";
	// Rope r8;
	// std::string s11 = s10.substr(0,12);
	// std::string s12 = s10.substr(12,12);
	// r7.split(12,r8);
	// r7.print_strings(std::cout);
	// r8.print_strings(std::cout);
	// std::cout<<"Passed index 12 test!"<<std::endl;
}
